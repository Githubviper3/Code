...
class Tilemap:
    ...
    def save(self, base=baseMapPath):
        self.savedlevelssize = str(len(os.listdir("Maps")) + 1)
    ...
    def render(self, surf, offset=(0, 0),blur = False):
        if not blur:
        ...
        for x in range(offset[0] // self.tile_size, (offset[0] + surf.get_width()) // self.tile_size + 1):
            for y in range(offset[1] // self.tile_size, (offset[1] + surf.get_height()) // self.tile_size + 1):
                ...
                if loc in self.tilemap:
                    ...
                    img.set_alpha(self.alpha)
                    if blur:
                        img = pygame.transform.gaussian_blur(img, 2)
...
class BackgroundTiles:
    ...
    def save(self,base=baseBGPath):
        self.savedlevelssize = str(len(os.listdir(base[:-1]))+1)
        filename = base + self.savedlevelssize + ".json"
        f = open(filename, 'w')
        json.dump({'tilemap': self.tilemap, 'tile_size': self.tile_size, 'offgrid': self.offgrid_tiles}, f)
        f.close()
        ...
        for x in range(offset[0] // self.tile_size, (offset[0] + surf.get_width()) // self.tile_size + 1):
            for y in range(offset[1] // self.tile_size, (offset[1] + surf.get_height()) // self.tile_size + 1):
                ...
                if loc in self.tilemap:
                    ...
                    img = self.game.assets[tile['type']][tile['variant']]
                    if blur:
                        img = pygame.transform.box_blur(img, 2,True)
                    img.set_alpha(self.alpha)
                    surf.blit(img, (
                        tile['pos'][0] * self.tile_size - offset[0], tile['pos'][1] * self.tile_size - offset[1]))
...
class Editor_layers:
    ...
    def save(self):
        self.level.save()
        self.extra.save(base="ExtraMaps/")
